...
MAIN_DIR <- substr(path, 1, unlist(stringr::str_locate_all(pattern ='/data/', path))[1])
ACT_DIR <- substr(path, 1, unlist(stringr::str_locate_all(pattern ='/new/', path))[2])
...
#Load behavioral 
memmatch <- read.csv(paste0(MAIN_DIR, "/data/behavioral/MemMatch_summary.csv"))
...
d <- merge(all_data, memmatch, by = "record_id")

write.csv(d, file = paste0(MAIN_DIR,"/data/summary/combined_data_", format(Sys.time(), "%Y-%m-%d"), ".csv"), row.names = FALSE)
```
