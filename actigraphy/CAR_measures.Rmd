...
for (i in 1:length(files)) {
  ...
  if (round(difftime(tail(act$datetime, 1), head(act$datetime, 1))) < act_period) {
    ...
    write.table(act_all, file = paste0(ACT_DIR, "/text/", record_id[i],".txt"), col.names = FALSE, row.names = FALSE, sep = " ")
  }
...
files <- list.files("~/Box/CogNeuroLab/Aging Decision Making R01/Data/Actigraphy/processed", pattern= ".csv", full.names = TRUE)
record_id <- substr(basename(files),1, 5)

#Get IS, IV, RA, L5, and M10 values
crdata <- nparACT_base_loop(files, SR = 2/60)
record_id <- unique(record_id)
crdata <- cbind(record_id, crdata)
#base_data <- base_data[c("record_id","IS", "IV", "RA", "L5", "L5_starttime", "M10", "M10_starttime")]

write.csv(crdata, file = paste0("~/Box/CogNeuroLab/Aging Decision Making R01/Data/cr/nparact_", format(Sys.time(), "%Y-%m-%d"), ".csv"), row.names = FALSE)
```
