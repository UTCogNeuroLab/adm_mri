...
vol <- read.csv('stats_vol_full.csv')
...
Available on Github: [mcmahonmc](https://github.com/UTCogNeuroLab/aging-study/tree/master/analysis/hippocampal-differentiation_neuroanat_spring-2021)
...
![Grady et al., 2020](grady.png){width=700px}

![Grady et al., 2020](grady_neurosynth.png){width=700px}
...
![Langnes et al., 2020](Langnes_2020.jpeg){width=700px}

![](langnes_2.png){width=700px}
...
tab1 <- CreateTableOne(vars = myVars, strata = 'Group' , data = df, factorVars = catVars)
...
![](Brainlife_Aging_Study.jpeg){width=700px}
...
## Manual labelling
![Eight coronal slices from Case 14 and corresponding manual annotations](1-s2.0-S1053811915003420-gr2.jpg)


![In vivo dataset and comparison with ex vivo images](1-s2.0-S1053811915003420-gr3.jpg)


![Freesurfer HBT](FS_HC_Parcellation.jpeg){width=700px}
...
d %>%
  ...
  facet_wrap(. ~ `Hippocampal region`, scales = 'free', nrow=3, ncol=1) + 
...
ICVnorm <- function(df, x){
  
  f <- paste(x, '~ EstimatedTotalIntraCranialVol')
  b <- as.numeric((lm(f, df))$coefficients[2])
  df[x] = df[x] - (b * (df$EstimatedTotalIntraCranialVol - mean(df$EstimatedTotalIntraCranialVol, na.rm = TRUE)))
...
d_old <- d
ya <- d_old %>% filter(Group == "Young Adults")
oa <- d_old %>% filter(Group == "Older Adults")
ya_old <- ya
oa_old <- oa
...
d_og <- d
d_og$Group <- factor(d_og$Group, levels = c('Young Adults', 'Older Adults'))
...
pl1 <- d_old %>%
  group_by(Group) %>%
  select(record_id, Group, Left.Hippocampus) %>%
  do(head(., 5))
...
ggplot() +
  geom_point(data = pl1, aes(x = 0, y = Left.Hippocampus, color = factor(record_id), group = Group)) +
  geom_point(data = pl2, aes(x = 1, y = Left.Hippocampus, color = factor(record_id), group = Group)) +
  geom_point(data = pl3, aes(x = 2, y = Left.Hippocampus, color = factor(record_id), group = Group)) + 
  theme_minimal() +
  scale_x_continuous(breaks = c(0, 1, 2), labels = c("no ICV adjustment", "ICV adjusted - all", "ICV adjusted - group")) + 
  scale_color_discrete(name="Participant ID") +
  ylab("Left hippocampal volume") +
  xlab("ICV adjustment method") +
  facet_grid(Group ~ .)
```
...
dmem %>%
  ...
  rename(`Tail` = Hippocampal_tail) %>%
  rename(`Total` = Hippocampus) %>%
  melt(id.vars = c('record_id', 'Group', 'cvlt_1', 'cvlt_sdelay_recall', 'cvlt_ldelay_recall', 'cvlt_recognition_hits'), measure.vars = c('Head', 'Tail', 'Total'), variable.name = 'Hippocampal region', value.name = 'Hippocampal subregion volume') %>%
...
dmem %>%
  ...
  rename(`Tail` = Hippocampal_tail) %>%
  rename(`Total` = Hippocampus) %>%
  melt(id.vars = c('record_id', 'Group', 'acc_mean.learning'), measure.vars = c('Head', 'Tail', 'Total'), variable.name = 'Hippocampal region', value.name = 'Hippocampal subregion volume') %>%
...
dmem %>%
  ...
  rename(`Tail` = Hippocampal_tail) %>%
  rename(`Total` = Hippocampus) %>%
  melt(id.vars = c('record_id', 'Group', 'acc_mean.test'), measure.vars = c('Head', 'Tail', 'Total'), variable.name = 'Hippocampal region', value.name = 'Hippocampal subregion volume') %>%
  ...
  ggsave("~/Desktop/hcvol_memtask.png")
...
dmem %>%
  ...
  rename(`Tail` = Hippocampal_tail) %>%
  rename(`Total` = Hippocampus) %>%
  melt(id.vars = c('record_id', 'Group', 'rt_c_mean.learning'), measure.vars = c('Head', 'Tail', 'Total'), variable.name = 'Hippocampal region', value.name = 'Hippocampal subregion volume') %>%
...
dmem %>%
  ...
  rename(`Tail` = Hippocampal_tail) %>%
  rename(`Total` = Hippocampus) %>%
  melt(id.vars = c('record_id', 'Group', 'rt_c_mean.test'), measure.vars = c('Head', 'Tail', 'Total'), variable.name = 'Hippocampal region', value.name = 'Hippocampal subregion volume') %>%
...
dmem %>%
  ...
  facet_grid(. ~ `Task Phase`, scales = 'free') + 
  scale_color_brewer(palette = 'Set1') + theme_minimal() + 
  xlab('Acrophase') + ggsave("~/Desktop/acrophase_rtc.png")
...
dmem %>%
  ...
  rename(`Tail` = Hippocampal_tail) %>%
  rename(`Total` = Hippocampus) %>%
  melt(id.vars = c('record_id', 'Group', 'accuracy.match', 'accuracy.mismatch'), measure.vars = c('Head', 'Tail', 'Total'), variable.name = 'Hippocampal region', value.name = 'Hippocampal subregion volume') %>%
...
dmem %>%
  ...
  rename(`Tail` = Hippocampal_tail) %>%
  rename(`Total` = Hippocampus) %>%
  melt(id.vars = c('record_id', 'Group', 'rt_c.match', 'rt_c.mismatch'), measure.vars = c('Head', 'Tail', 'Total'), variable.name = 'Hippocampal region', value.name = 'Hippocampal subregion volume') %>%
...
dmem %>%
  ...
  rename(`Tail` = Hippocampal_tail) %>%
  rename(`Total` = Hippocampus) %>%
  melt(id.vars = c('record_id', 'Group', 'memrepeat_accuracy'), measure.vars = c('Head', 'Tail', 'Total'), variable.name = 'Hippocampal region', value.name = 'Hippocampal subregion volume') %>%
...
dmem %>%
  ...
  rename(`Tail` = Hippocampal_tail) %>%
  rename(`Total` = Hippocampus) %>%
  melt(id.vars = c('record_id', 'Group', 'memrepeat_rt'), measure.vars = c('Head', 'Tail', 'Total'), variable.name = 'Hippocampal region', value.name = 'Hippocampal subregion volume') %>%
...
summary(lm(accuracy.match ~ Hippocampal_tail + Group, data = dmem)) 

summary(lm(rt_c.mismatch ~  Hippocampal_head * Group, data = dmem)) 
summary(lm(rt_c.match ~ Hippocampal_head + Group, data = dmem)) 

summary(lm(rt_c.mismatch ~  Hippocampal_tail * Group, data = dmem)) 
summary(lm(rt_c.match ~ Hippocampal_tail + Group, data = dmem)) 
...
cordf <- dmem2 %>%
  select(actamp:fact, IS, IV, RA, matches("Amygdala|Caudate|Pallidus|Putamen|Thalamus")) %>%
  corrr::correlate() %>%
  corrr::focus(actamp:fact, IS, IV, RA)
...
dmem2 %>%
  select(actamp:fact, IS, IV, RA, matches("Amygdala|Caudate|Pallidus|Putamen|Thalamus")) %>%
  corrr::correlate() %>%
  corrr::rplot() + theme_minimal() + theme(axis.text.x = element_text(angle = 90)) +
  ggsave('~/Desktop/corplot.png')
