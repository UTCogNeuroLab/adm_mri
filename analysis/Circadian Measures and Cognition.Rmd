...
p3 <- ggplot(data = d) + 
  geom_point(aes(x = actdownmesor, y = fpn_modularity_x, group = group, color = group), size=2) + 
  stat_smooth(aes(x = actdownmesor, y = fpn_modularity_x, group = group, color = group), method = 'lm', formula = y ~ poly(x, 2), se = FALSE, fullrange = FALSE) +
  theme_minimal() +
  scale_color_manual(values = brewer.pal(8, "Paired")[7:8]) +
  xlab('Down-Mesor') + ylab(element_blank())

ggarrange(p1, p2, p3, heights = c(3, 3, 3), widths = c(4, 4, 7),
          ncol = 3, nrow = 1)
