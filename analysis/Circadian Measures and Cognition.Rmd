...
d$groupname <- factor(ifelse(d$record_id < 40000, 0, 1), labels = c('Young Adults', 'Older Adults'))
...
d.mlt <- melt(select(d, record_id, group, groupname, IS:fact, -rsqact, -fnlrgact, -L5_starttime, -M10_starttime), id.vars=c('record_id', 'groupname', 'group'))

ggplot(data = d.mlt, aes(x = variable, y = value, group = groupname)) + 
  geom_violin(aes(fill = groupname)) + 
...
#Older Adults
summary(lm(actamp ~ age, data = d[d$group == 1,])) #
summary(lm(fact ~ age, data = d[d$group == 1,])) #
summary(lm(actupmesor ~ age, data = d[d$group == 1,])) #
...
summary(lm(IV ~ age, data = d[d$group == 1,])) #NS

plot(d$age[d$group == 1], d$actamp[d$group == 1], xlab = "Age", ylab = "Amplitude")
plot(d$age[d$group == 1], d$fact[d$group == 1], xlab = "Age", ylab = "F-statistic")
plot(d$age[d$group == 1], d$actupmesor[d$group == 1], xlab = "Age", ylab = "Up-Mesor")
...
plot(d$age[d$group == 0], d$actupmesor[d$group == 0], xlab = "Age", ylab = "Up-Mesor")
plot(d$age[d$group == 0], d$actdownmesor[d$group == 0], xlab = "Age", ylab = "Down-Mesor")
...
plot(d$IV, d$fpn_efficiency_x, xlab = "Intradaily Variability", ylab = "FPN Efficiency")
#---------------------------------------------------------
...
ggplot(data = d) + 
  ...
  geom_point(aes(x = actamp, y = dmn_participation_x, group = groupname, color = groupname), size=2) + 
  stat_smooth(aes(x = actamp, y = dmn_participation_x, group = groupname, color = groupname), method = 'lm', se = FALSE, fullrange = FALSE) +
  theme_minimal() +
  scale_color_manual(values = brewer.pal(8, "Paired")[7:8]) +
...
ggplot(data = d) + 
  ...
  geom_point(aes(x = IS, y = dmn_participation_x, group = groupname, color = groupname), size=2) + 
  stat_smooth(aes(x = IS, y = dmn_participation_x, group = groupname, color = groupname), method = 'lm', se = FALSE, fullrange = FALSE) +
  theme_minimal() +
  scale_color_manual(values = brewer.pal(8, "Paired")[7:8]) +
...
ggplot(data = d) + 
  ...
  geom_point(aes(x = sleep_time, y = dmn_participation_x, group = groupname, color = groupname), size=2) + 
  stat_smooth(aes(x = sleep_time, y = dmn_participation_x, group = groupname, color = groupname), method = 'lm', se = FALSE, fullrange = FALSE) +
  theme_minimal() +
  scale_color_manual(values = brewer.pal(8, "Paired")[7:8]) +
...
ggplot(data = d) + 
  ggtitle('DMN Participation Coefficient and F-Statistic') + 
  geom_point(aes(x = fact, y = dmn_participation_x, group = group, color = group), size=2) + 
  stat_smooth(aes(x = fact, y = dmn_participation_x, group = group, color = group), method = 'lm', formula = y ~ poly(x, 3), se = FALSE, fullrange = FALSE) +
  theme_minimal() +
  scale_color_manual(values = brewer.pal(8, "Paired")[7:8]) +
  xlab('F-statistic') + ylab('DMN Participation Coefficient')

lm1 <- lm(dmn_participation_x ~ fact, data = d[d$group == 1, ])
AIC(lm1)
d_n <- dplyr::select(d, dmn_participation_x, fact, group)
d_n <- d_n[complete.cases(d_n) == TRUE,]

lm2 <- lm(dmn_participation_x ~ poly(fact, 3) + group, data = d_n)
AIC(lm2)

d_n <- dplyr::select(d, fact, cvlt_sdelay_recall_zscore)
d_n <- d_n[complete.cases(d_n) == TRUE,]

lm2 <- lm(cvlt_sdelay_recall_zscore ~ poly(fact, 3), data = d_n)
summary(lm2)
AIC(lm2)
