...
d <- select(d, -fa_mean_3, -fa_mean_4, -fa_mean_5, -md_mean_3, -md_mean_4, -md_mean_5)
...
d %>%
  select(Group, md_mean_3_skel, md_mean_4_skel, md_mean_5_skel) %>%
  melt(id.vars = "Group") %>%
  ggplot() +
...
d %>%
  select(Group, fa_mean_3_skel, fa_mean_4_skel, fa_mean_5_skel) %>%
  melt(id.vars = "Group") %>%
  ggplot() +
  geom_histogram(aes(x = value, fill = variable)) +
  scale_fill_discrete(name = "ROI", labels = c("Genu", "Body", "Splenium")) +
...
corrplot(ya_mat, p.mat = ya_res$p, sig.level = alpha, insig = "blank", type = "upper", addCoef.col = TRUE, pch.cex = 2)

oa_mat <- cor(select(oa_data, -record_id, -files, -Group))
oa_res <- cor.mtest(oa_mat, conf.level = (1-alpha))
corrplot(oa_mat, p.mat = oa_res$p, sig.level = alpha, insig = "blank", type = "upper", addCoef.col = TRUE)
